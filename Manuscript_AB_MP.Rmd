...
To further constraint the research questions, Experiment 2 was parallel to Experiment 1, except for the use of a very short prime exposure duration (i.e., 16.6ms). Similar prime exposure durations have shown rather weak masked priming lexical decision experiments in a laboratory setting: smaller than 5 ms for a prime durations of 14 ms [@ziegler2000] and smaller than 9 ms for a prime duration of 20 ms [@Tzur_2007]---in the @Tzur_2007 experiment, this difference increased to 16 ms when using a very high level of contrast in the computer screen. Thus, if prime exposure durations in online masked priming correspond to the nominal ones, we would expect a sizeable priming effect at the 33.3 but only a residual priming effect at the 16.6 ms prime exposure duration. Alternatively, if we obtain a sizeable effect at the 16.6 ms prime exposure duration (e.g., above 20-25 ms), this would indicate, again, that we should be wary of using the masked priming technique in online experiments. Keep in mind that we are using a software that has good control over the exposure duration (\color{red} CITE).
...
We report here how we determined our sample size, all data exclusions (if any), all manipulations, and all measures in the study. <!-- 21-word solution (Simmons, Nelson & Simonsohn, 2012; retrieved from http://ssrn.com/abstract=2160588) -->
...
e1_actual_participants <- filter(e1_all_participants, PROLIFIC_PID %in% e1$participant & `What is your age?` != 99 & `Response ID` != "R_sU4qJ6UCe9jRKs9")
...
# there aren't actually any of these, but will leave the code in just in calse
other_participants_nr <- filter(e1_actual_participants, `What is your gender (optional)?` == "Other") %>% nrow

NA_participants_nr <- filter(e1_actual_participants, is.na(`What is your gender (optional)?`)) %>% nrow

#coordinates(e1_actual_participants) <- c("Location Longitude","Location Latitude")
...
participant_coords <- e1_actual_participants %>% select("Location Longitude", "Location Latitude")

e1_actual_participants$country <- coords2country(participant_coords)

e1_participant_countries <- table(e1_actual_participants$country %>% as.character) #make into a character vector since the original output is a factor with as many levels as there are countries
```

### Participants

We collected data from `r e1_actual_participants$PROLIFIC_PID %>% unique %>% length` participants aged from `r participant_ages %>% min` to `r participant_ages %>% max` (mean age: `r participant_ages %>% mean %>% round(2)`) recruited through Prolific [www.prolific.co, -@prolific2021]. Of the participants, `r male_participants_nr` identified as male and `r female_participants_nr` identified as female. The experiment was only shown to participants who indicated that English was their first language in the Prolific screening questions. Based on their IP addresses, `r e1_participant_countries["United Kingdom"] %>% ifelse(. < 11, numbers_to_words(.), .)` participants were based in the UK, `r e1_participant_countries["United States of America"] %>% ifelse(. < 11, numbers_to_words(.), .)` were based in the US, `r e1_participant_countries["Canada"] %>% ifelse(. < 11, numbers_to_words(.), .)` participants were based in Canada, and `r e1_participant_countries["Ireland"] %>% ifelse(. < 11, numbers_to_words(.), .)` participants were based in Ireland. Two participants could not be localized in this way. Participants who completed the experiment were paid £1.25 for their participation (corresponding to £5/hour). All participants were naïve to the purpose of the experiment. Participants could use a desktop or laptop computer or a mobile device. Because of a technical display issue with PsychoJS and the Safari browser, participants who tried to access the experiment using that browser, including all participants on iOS devices, were advised to change browser or device and restart the experiment.
...
```{r exp1_descriptive_rt_table, as.is = TRUE,echo=FALSE}
#load raw Exp1 data
exp1 <- read_csv("Exp 1 data.csv")
exp1$device <- ifelse(exp1$OS %in% c("Linux armv7l", "Linux armv8l"), "android", "computer")

exp1$StimulusType <- factor(exp1$StimulusType, levels = c("NW", "WORD"), labels = c("Nonword", "Word"))
exp1$Condition <- factor(exp1$Condition, levels = c("ID", "UN"), labels = c("Identical", "Unrelated"))
exp1$PrimeDuration <- factor(exp1$PrimeDuration, levels = c(33, 50), labels = c("33 ms", "50 ms"))


exp1_descriptives_rt <- exp1 %>%
...
(ref:exp1-blmm-table-caption) Posterior mean, standard error, 95% credible interval and $\hat{R}$ statistic for the fixed effects of the model fitted for correct word response times in Experiment 1.


```{r load_exp1_brms_data, echo=FALSE, as.is = TRUE}
...
blmm_exp1_summary <- summary(blmm_exp1_rt)

blmm_exp1_fixed_for_print <- blmm_exp1_summary$fixed
...
blmm_exp1_fixed_for_print <- blmm_exp1_fixed_for_print %>% tibble(Parameter = parameter_names, .)

colnames(blmm_exp1_fixed_for_print) <- c("mean","SE","lower bound","upper bound","Rhat", "Bulk ESS","Tail ESS")

blmm_exp1_fixed_for_print <- blmm_exp1_fixed_for_print  %>%
  dplyr::select(-ends_with("ESS"))

write.csv(blmm_exp1_fixed_for_print, file = "Table_1.csv", row.names = FALSE)

apa_table(
    blmm_exp1_fixed_for_print,
    placement = "H",
    align = c("c", "c", "c", "c", "c"),
...
blmm_exp2_summary <- summary(blmm_exp2_rt)

blmm_exp2_fixed_for_print <- blmm_exp2_summary$fixed
...
blmm_exp1_fixed_for_print <- blmm_exp2_fixed_for_print %>% tibble(Parameter = parameter_names, .)

colnames(blmm_exp2_fixed_for_print) <- c("mean","SE","lower bound","upper bound","Rhat", "Bulk ESS","Tail ESS")

blmm_exp2_fixed_for_print <- blmm_exp2_fixed_for_print  %>%
  dplyr::select(-ends_with("ESS"))

write.csv(blmm_exp2_fixed_for_print, file = "Table_3.csv", row.names = FALSE)
