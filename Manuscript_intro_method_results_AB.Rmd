...
plot(del_hdi_exp1_rt) +
  scale_fill_flat() + theme_apa()

#scale_fill_manual(values = c("dodgerblue4","dodgerblue3","dodgerblue2","deepskyblue1", "lightskyblue"), labels = c("50%", "75%", "85%", "95%", "100%"))+

```

To better visualize the data, we computed the delta plots for the effect of Priming (see Panel A in Figure \@ref(fig:Deltas_E1_Words)), as well as for the effect of Prime Duration (see Panel B in Figure \@ref(fig:Deltas_E1_Words)). These delta plots are residual quantile plots that show the size of the effect for the .1, .3, .5, .7, and .9 response time quantiles.

(ref:Deltas_E1_Words-caption) Delta in Experiment 1...EXPLAIN.

```{r Deltas_E1_Words, echo=FALSE, cache=TRUE, fig.cap = "(ref:exp1-brms-rt-plot-caption)"}
...
#plot(del_hdi_exp2_rt_dist) + scale_fill_flat() + theme_apa()
```
