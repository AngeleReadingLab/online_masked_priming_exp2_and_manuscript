...
```{r model_parameters_table, warning=FALSE, echo=FALSE, message=FALSE}
Experiments <-c("1","2")
...
Parameters <-c( "$T_{er}$ values for Experiment 1.  33ms nonword ID", "$T_{er}$ values for Experiment 1. 33ms nonword Unrel",
                "$T_{er}$ values for Experiment 1.  50ms nonword ID", "$T_{er}$ values for Experiment 1. 50ms nonword Unrel",
                "$T_{er}$ values for Experiment 1.  33ms word ID", "$T_{er}$ values for Experiment 1. 33ms word Unrel",
                "$T_{er}$ values for Experiment 1.  50ms word ID", "$T_{er}$ values for Experiment 1. 50ms word Unrel")
...
Ratcliff, R.,\& Tuerlinckx, F. (2002). Estimating the parameters of the diffusion model: Approaches to dealing with contaminant reaction times and parameter variability. Psychonomic Bulletin and Review, 9, 438–481. https://doi.org/10.3758/BF03196302
